...
pub fn get_recent_sync_events(conn: &Connection, limit: usize) -> Result<Vec<SyncLogEntry>> {
    let logs =
        core_lib::storage::get_recent_sync_logs(conn, limit).map_err(|e| anyhow::anyhow!(e))?;
    Ok(logs)
}
...
pub fn print_status_summary(
    ...
    if peer_count == 0 {
        ...
        let first_three: Vec<String> = peers.peers.iter().take(3).map(|p| p.url.clone()).collect();
        let suffix = if peer_count > 3 {
            format!(" (+{} more)", peer_count - 3)
        } else {
            String::new()
        };
        println!("{} {}{}", "Peers:".blue(), first_three.join(", "), suffix);
    }
    ...
    for l in logs.iter() {
        ...
        // Строка события
        println!("#{} {} {} {} {}", l.id, ts, l.peer_url, l.mode, icon);
