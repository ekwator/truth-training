...
@Dao
interface ImpactDao {
    @Query("SELECT * FROM impacts WHERE event_id = :eventId ORDER BY created_at DESC")
    suspend fun listImpactsForEvent(eventId: String): List<ImpactEntity>
    
    @Query("SELECT * FROM impacts WHERE event_id = :eventId ORDER BY created_at DESC")
    fun listImpactsForEventFlow(eventId: String): Flow<List<ImpactEntity>>
    
    @Query("SELECT * FROM impacts WHERE id = :id")
    suspend fun getImpactById(id: String): ImpactEntity?
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertImpact(impact: ImpactEntity)
