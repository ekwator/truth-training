...
interface JudgmentDao {
    ...
    suspend fun listJudgmentsForEvent(eventId: Long, limit: Int, offset: Int): List<JudgmentEntity>

    @Query("SELECT * FROM judgments WHERE event_id = :eventId ORDER BY submitted_at DESC")
    fun listJudgmentsForEventFlow(eventId: Long): Flow<List<JudgmentEntity>>

    @Query("SELECT COUNT(*) FROM judgments WHERE event_id = :eventId")
    suspend fun countJudgmentsForEvent(eventId: Long): Int
    ...
    suspend fun countJudgmentsByAssessment(eventId: Long, assessment: String): Int

    @Query("SELECT AVG(confidence_level) FROM judgments WHERE event_id = :eventId")
    suspend fun averageConfidence(eventId: Long): Double?
}
