...
-- Trigger to update impact_score when impact is deleted
CREATE TRIGGER update_impact_score_after_impact_delete
AFTER DELETE ON impact
BEGIN
    UPDATE truth_event 
    SET impact_score = (
        SELECT 
...
-- Trigger to update judgment_score when judgment is deleted
CREATE TRIGGER update_judgment_score_after_judgment_delete
AFTER DELETE ON judgment
BEGIN
    UPDATE truth_event 
    SET judgment_score = (
        SELECT 
        ...
        LEFT JOIN participants p ON j.participant_id = p.public_key
        WHERE ec.created_by = (
            SELECT created_by FROM event_ci WHERE id = OLD.event_id
        )
    )
    WHERE id = (
        SELECT created_by FROM event_ci WHERE id = OLD.event_id
