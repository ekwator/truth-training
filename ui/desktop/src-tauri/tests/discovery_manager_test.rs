...
async fn test_discovery_manager_initialization() {
    ...
    let conn = Arc::new(TokioMutex::new(
        core_lib::storage::open_db(&db_path.display().to_string())
    ...
    // Verify connection works
    {
        ...
        let result: i64 = guard.query_row("SELECT 1", rusqlite::params![], |row| row.get(0))
    ...
    {
        let guard = conn.lock().await;
        let mut stmt = guard.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='nodes'")
            .expect("Failed to prepare statement");
        let exists: bool = stmt.exists(rusqlite::params![]).expect("Failed to check table");
...
async fn test_node_storage_operations() {
    ...
    // Verify node was stored
    {
        ...
        let nodes = core_lib::storage::list_nodes(&guard, &filter)
...
async fn test_ttl_cleanup_logic() {
    ...
    let now = chrono::Utc::now().timestamp();
    {
        ...
        let pruned = core_lib::storage::prune_stale_nodes(&guard, now)
            .expect("Failed to prune stale nodes");
