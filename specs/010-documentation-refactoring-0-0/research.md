...
**Decision**: Walk the repo with `Path.rglob('*.md')`, skip `CONTRIBUTING.md`, `LICENSE.txt`, `SECURITY.md`, `CHANGELOG.md`, and coerce any `README*.md` depth to 0. Depth derives from the number of path segments under repo root, capped at 3 to keep navigation shallow.  
...
**Decision**: Introduce `spec_optimizer.py` that enforces <80-word paragraphs, mandates "Use /spec before /docs" admonition in `spec/README.md`, and keeps decision/context sections in bullet lists. `/docs` receives the detailed sections extracted from README + specs, with archive relocation for pre-v1.0.0 materials.  
