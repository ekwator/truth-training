...
pub async fn run_diagnostics(
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            results.push(ServerDiagnosticResult {
                check: "API".into(),
                status: "❌ Error".into(),
    ...
    match client.get(&url).send().await {
        ...
        Ok(resp) => {
            results.push(ServerDiagnosticResult {
                check: "API".into(),
                status: "❌ Error".into(),
                message: format!("{} => {}", url, resp.status()),
            });
        }
        Err(e) => {
            results.push(ServerDiagnosticResult {
    ...
    match core_lib::storage::open_db(db_path) {
        Ok(conn) => {
            ...
            match core_lib::storage::load_metrics(&conn) {
                Ok(_) => results.push(ServerDiagnosticResult {
                    check: "Database".into(),
                    status: "✅ OK".into(),
                    message: db_path.to_string(),
                }),
                Err(e) => results.push(ServerDiagnosticResult {
                    check: "Database".into(),
                    status: "❌ Error".into(),
                    message: e.to_string(),
                }),
    ...
    if !p2p_enabled {
        ...
        match std::net::UdpSocket::bind(bind_addr) {
            Ok(sock) => {
                ...
                results.push(ServerDiagnosticResult {
                    check: "P2P".into(),
                    status: "⚠️ Inactive".into(),
                    message: "UDP 37020 is free".into(),
                });
            }
            Err(e) => {
