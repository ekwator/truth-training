...
impl CryptoIdentity {
    ...
    #[allow(dead_code)]
    // Эта функция используется из CLI (`truthctl`) и может не вызываться
    // внутри самого крейта `truth_core` при сборке без соответствующих
    // фич, поэтому локально подавляем предупреждение dead_code.
