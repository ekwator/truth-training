...
pub fn extract_envelope(req: &HttpRequest) -> Result<TransportEnvelope, HttpResponse> {
    let pk = req.headers().get("X-Public-Key").and_then(|v| v.to_str().ok());
    let sig = req.headers().get("X-Signature").and_then(|v| v.to_str().ok());
    let ts = req.headers().get("X-Timestamp").and_then(|v| v.to_str().ok());
    match (pk, sig, ts) {
        (Some(pk), Some(sig), Some(ts)) if !pk.is_empty() && !sig.is_empty() && !ts.is_empty() => Ok(TransportEnvelope { public_key: pk.to_string(), signature: sig.to_string(), timestamp: ts.to_string() }),
